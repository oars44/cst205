...
img_path = os.path.dirname(os.path.abspath(__file__)) + '/Big_cards/'
...
if PiOrUSB == 2:
    ...
    while not cap.isOpened():  # Cycle through all available indexes until external webcam is found
        print("cam not found")
        cam += 1
        cap = cv2.VideoCapture(cam)

    #cap.set(cv2.CAP_PROP_EXPOSURE, -8)  # Decrease camera exposure to help see cards
