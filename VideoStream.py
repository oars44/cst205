...
# acquisition of frames from a USB camera. The object uses
...
class VideoStream:
    ...
    def __init__(self, resolution=(640, 480), framerate=30, PiOrUSB=1, src=3):
        ...
        if self.PiOrUSB == 2:  # USB camera
            ...
            while not self.stream.isOpened():  # Cycle through all available indexes until external webcam is found
                print("cam not found")
                cam += 1
                self.stream = cv2.VideoCapture(cam)

            self.stream.set(cv2.CAP_PROP_EXPOSURE, -8)  # Decrease camera exposure to help see cards
